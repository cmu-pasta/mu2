...
elif [ $FUZZ_CONFIG == "mu2AndZest" ]; then
    (cd $HOME/jqf; git checkout vasu/jqf-1.9-favormu2; git pull; mvn install > /dev/null && echo "Built: JQF master for Mu2")
    FUZZ_SCRIPT="scripts/runMu2WithZest.sh"
